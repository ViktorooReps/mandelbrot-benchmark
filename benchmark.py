...
if __name__ == '__main__':
    ...
    for version_id, n in tqdm(product(version_ids, all_ns), total=len(version_ids) * len(all_ns)):
        result = subprocess.run(
            [args.exe_path, version_id, '-2', '1', '-1.5', '1.5', str(n), str(n), '100'],
            stdout=subprocess.PIPE,
            stderr=subprocess.DEVNULL,
            text=True
        )

        ns, ns_op = re.findall(REGEX, result.stdout)
