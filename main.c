...
void save_matrix(uint32_t *matrix, int32_t n_rows, int32_t n_cols,
                 const char *filename) {
  ...
  // write the dimensions of the matrix to the file (to be able to reconstruct
  // it later)
...
struct OpStats naive_mandelbrot(uint32_t *n_iterations, double lower_real,
                                double upper_real, double lower_imaginary,
                                double upper_imaginary, int height, int width,
                                int max_iterations) {
  ...
  for (int y = 0; y < height; ++y) {
    ...
    for (int x = 0; x < width; ++x) {
      ...
      for (n = 0; n < max_iterations; ++n) {
        ...
        // z_{i + 1} = z_i ** 2 + c = (re(z_i) ** 2 - im(z_i) ** 2, 2 * re(z_i)
        // * im(z_i))
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 8) {
    printf("Usage: %s <lower_rational> <upper_rational> <lower_irrational> "
           "<upper_irrational> <height> <width> <max_iterations>\n",
           argv[0]);
    return 1;
  }
  ...
  struct OpStats result =
      naive_mandelbrot(n_iterations, lower_real, upper_real, lower_imaginary,
